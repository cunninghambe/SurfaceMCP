...
# 10. Addendum — `auth.kind: 'oauth2'` secret handling (2026-07-25)

The OAuth2 client-credentials kind (see [SPEC_OAUTH2_AUTH.md](SPEC_OAUTH2_AUTH.md)) introduces two new classes of secret — a **client secret** and the **access/refresh tokens** minted from it. It inherits the posture above; the kind-specific rules are:

- **Provenance.** `client_id` / `client_secret` live in `roles[].credentials` behind `$env:` indirection, like every other kind. The `auth` block itself holds no secrets, so a committed `surfacemcp.config.json` stays clean. §2's "secret sprawl" control (`findLiteralCredentialPaths` at `loadConfig` + in `doctor`) covers the new kind unchanged, because it is auth-kind agnostic by construction.
- **Transit.** The client secret is sent only in the token request — HTTP Basic header (RFC 6749 §2.3.1, each half form-urlencoded before base64) or form body when `clientAuth: 'body'`. Never in a URL or query string. The token request uses `redirect: 'manual'` for the same reason `executeCall` does: a 3xx must never replay the credentials to an attacker-chosen host. `doctor` warns when `tokenUrl` is neither https nor loopback, and its reachability probe is a **GET** so it never transmits credentials.
- **Logging.** Nothing secret reaches pino. The oauth2 paths log only role names, refresh counts and HTTP status codes — the repo convention. `src/auth/oauth2.test.ts` asserts this directly: it runs a full login → proactive-expiry → refresh-grant → `executeCall` cycle with sentinel secret values and fails if any captured log record contains them.
- **Error strings.** A non-2xx token response is reported as `status <code>` plus the RFC 6749 §5.2 `error` code, and only when that code matches `/^[a-z_]{1,64}$/`. The response body is never echoed — `error_description` is authorization-server-authored free text that has been observed to quote request parameters.
- **Disclosure via `surface_describe_auth`.** The `oauth2` variant is hard-coded `redacted: true`: it returns the token-request shape, the credential-key → field map, and `valueMeta` (`present`/`length`/`source`) only. `revealSecrets: true` does **not** reveal the client secret for this kind — unlike form/nextauth there is no browser login to drive with it, so plaintext would be disclosure with no consumer. Access tokens are never described. `surface_login_status` continues to return only `cachedAt` / `lastRefreshAt` / `refreshCount`, so the new session fields (`token`, `tokenType`, `expiresAt`, `refreshToken`) never cross the MCP wire.
- **Header injection from a hostile AS.** `access_token` is required to be printable ASCII and `token_type` to be an RFC 7230 token (else it falls back to `Bearer`) before either is spliced into an outbound `Authorization` header.
- **Hostile/broken token endpoint.** `tokenUrl` is schema-restricted to `http(s)` and may not embed `user:password@` userinfo (credentials belong in `roles[].credentials`). The token response is refused past 64 KB rather than buffered, matching `executeCall`'s body cap, and the request carries a 15 s timeout.
